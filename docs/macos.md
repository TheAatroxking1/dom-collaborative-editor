...
每一步成功后再执行下一步。服务就绪后会自动打开局域网页面；如果列出多个候选地址，先在终端输入双方可访问的网卡地址编号并回车。点击「新建文档」，再把协作链接发给另一个浏览器或局域网内的另一台电脑。终端需要保持运行；按 **Control+C** 正常停止，并留意停机日志中的错误。
...
`sh scripts/serve.sh` 默认监听所有 IPv4 网卡（`0.0.0.0:5274`），服务就绪后打开检测到的局域网 IP；只有一个候选地址时自动使用，多个地址时在终端选择编号。没有候选地址时会明确提示并回退到本机 `127.0.0.1`，这个回退地址不能供其他电脑使用。
...
不希望打开浏览器或交互选择时，运行 `sh scripts/serve.sh --no-browser`，终端只显示候选访问地址。
已知要打开的 IP 时可改用 `sh scripts/serve.sh --browser-host 192.168.1.100`，替换为实际地址；两者不能同时使用。
没有可输入终端时，应明确使用其中一种。`--browser-host` 只指定浏览器地址，不会改变服务监听范围。

脚本通过共享的 `backend/serve.py` 启动。支持 `--host`、`--port`、`--ssl-certfile`、`--ssl-keyfile`、`--no-browser`、`--browser-host`、`--log-level`，不透传任意 Uvicorn 参数；worker 数固定为 1，连接排空超时固定为 10 秒。
...
默认打开的 HTTP 局域网地址不是安全上下文，支持在线协作和已打开页面断线后继续编辑；**不能整站离线刷新。该功能需要可信 HTTPS，并且页面与文档已缓存**。
...
协议、IP 或端口改变都会改变浏览器本地存储的来源。升级后默认打开局域网 IP；若以前在 `127.0.0.1` 或 `localhost` 下编辑，先回到旧地址完成同步或导出备份，再切换到新地址。具体步骤见 [换地址迁移指南](local-and-lan.md#三换地址后迁移未同步的内容)。
