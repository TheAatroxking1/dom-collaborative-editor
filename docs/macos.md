...
每一步成功后再执行下一步。打开 **<http://127.0.0.1:5274>**，点击「新建文档」，再用另一个浏览器打开同一协作链接。终端需要保持运行；按 **Control+C** 正常停止，并留意停机日志中的错误。

后续启动只需在仓库根目录执行 `sh scripts/serve.sh`，不需要 `chmod`。更新前端源码后重新执行 build；依赖文件变化后重新安装对应依赖。
...
在「系统设置 → 网络 → 当前连接的详细信息 → TCP/IP」查找服务电脑的 IP。例如地址为 `192.168.1.100`，两台电脑都访问 `http://192.168.1.100:5274`，在这个地址创建并分享文档。另一台设备只需要浏览器，不需要安装项目。

不要分享 `localhost`、`127.0.0.1` 或 `0.0.0.0`。无法连接时，检查 macOS 防火墙是否允许 Python 入站连接、设备间网络是否互通，以及 VPN、访客网络隔离和端口占用。
...
此时访问 `https://实际局域网IP:5274`。证书可按 [mkcert 官方说明](https://github.com/FiloSottile/mkcert) 生成；只在服务电脑安装根证书不会让其他设备自动信任它，不能通过忽略证书错误代替信任配置。私钥不要上传或分享。
...
打开 **<http://127.0.0.1:5273>**。开发版不提供页面离线缓存；验收离线刷新用 5274 构建版。分别在各终端按 Control+C 停止。
