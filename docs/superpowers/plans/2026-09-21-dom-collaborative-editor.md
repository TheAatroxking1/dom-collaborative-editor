...
> - **端口全部避开常见占用**：计划里开发用 5173/8000、端到端用 8000/8001/5174。
>   实际机器上 5173 被另一个项目的 Vite 占用、8000 被 Docker 与另一个服务占用。
>   按“不按端口结束未知进程”的要求，开发改为 5273（前端）与 8787（后端），
>   端到端改为 5473 与 8791；均可用环境变量覆盖。
