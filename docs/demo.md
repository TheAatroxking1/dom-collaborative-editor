...
真实双设备演示时，Windows 和 Mac 的启动入口默认都监听所有 IPv4 网卡。在本机页面点击「复制协作链接」会获取服务电脑的局域网地址；有多个地址时选择双方可访问的网卡地址，再发给另一台设备。生成链接不改变当前页面地址，仍需确认防火墙允许入站连接；参见各系统的运行指南。不要向另一台设备分享 localhost 链接。
